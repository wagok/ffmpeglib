...
package com.wagok.ffmpeg.ffmpeg;
...
public class FFmpegPlayer {
    ...
    private static class SaveTask extends
            AsyncTask<Object, Void, NotPlayingException> {

        private final FFmpegPlayer player;

        public SaveTask(FFmpegPlayer player) {
            this.player = player;
        }

        @Override
        protected NotPlayingException doInBackground(Object... params) {
            String url = (String) params[0];
            try {
                player.saveNative(url);
                return null;
            } catch (NotPlayingException e) {
                return e;
            }
        ...
        protected void onPostExecute(NotPlayingException result) {
            if (player.mpegListener != null)
                player.mpegListener.onFFSaved(result);
        }
	...
	static {
		...
		if (nativeTester.isNeon()) {
			...
			System.loadLibrary("com/wagok/ffmpeg");
    ...
    private native void saveNative(String fileName) throws NotPlayingException;
    ...
    public native void ffmpegCMD(String[] arr);
