...
int player_decode_audio(struct DecoderData * decoder_data, JNIEnv * env,
	...
	if (player->save) {
        ...
        pthread_mutex_lock(&player->mutex_save);
        write_audio_frame2(player->encoderData.oc, player->encoderData.audio_st, &player->encoderData, frame);
        pthread_mutex_unlock(&player->mutex_save);
        return 0;
 	}
...
void player_update_time(struct State *state, int is_finished) {
	...
	if (time_diff > 50000ll || time_diff < -50000ll) {
...
int player_decode_video(struct DecoderData * decoder_data, JNIEnv * env,
	...
	// injection
	if (player->save) {
        // save frame to encoder
        pthread_mutex_lock(&player->mutex_save);

        write_video_frame2(player->encoderData.oc, player->encoderData.video_st, &player->encoderData, frame);
        pthread_mutex_unlock(&player->mutex_save);
        player->encoded_frame++;
...
void * player_decode(void * data) {
	...
	for (;;) {
        ...
        if (player->save && packet_data->end_of_stream) {
			LOGI(1, "player_decode end of stream send signal");
			player->save = FALSE;
			pthread_cond_broadcast(&player->cond_finish);
		}
...
static void print_fps(double d, const char *postfix){
    ...
    if     (v% 100      )  {
    LOGI(1, ", %3.2f %s", d, postfix);}
    else if(v%(100*1000)) {LOGI(1, ", %1.0f %s", d, postfix);}
    else                 { LOGI(1, ", %1.0fk %s", d/1000, postfix);}
}

int player_fill_input_file_options(struct Player *player){


   AVStream *audio_st, *video_st;
   audio_st = player->input_streams[player->audio_stream_no];
   video_st = player->input_streams[player->video_stream_no];

   // Audio
   player->encoderData.audio_bit_rate = audio_st->codec->bit_rate;
   player->encoderData.audio_sample_rate = audio_st->codec->time_base.den;
   player->encoderData.audio_channels = audio_st->codec->channels;
   player->encoderData.audio_codec_id = audio_st->codec->codec_id;
   player->encoderData.audio_sample_fmt = audio_st->codec->sample_fmt;

   // Video
   player->encoderData.video_codec_id = video_st->codec->codec_id;
   player->encoderData.video_frame_rate = av_q2d(video_st->r_frame_rate);
   player->encoderData.video_bit_rate = video_st->codec->bit_rate;
   player->encoderData.video_width = video_st->codec->width;
   player->encoderData.video_height = video_st->codec->height;
   player->encoderData.duration = 0;
   if (player->input_format_ctx->duration != AV_NOPTS_VALUE) {
      player->encoderData.duration = player->input_format_ctx->duration / (AV_TIME_BASE/1000);
   }
   LOGI(1, "cccode: a_bit_rate:%d, a_sample_rate:%d, a_channels %d, a_codec_id: %d",
   player->encoderData.audio_bit_rate, player->encoderData.audio_sample_rate,
   player->encoderData.audio_channels, player->encoderData.audio_codec_id);
   LOGI(1, "cccode: v_codec_id %d, v_frame_rate: %d, v_bit_rate: %d, v_width: %d, v_height: %d, duration: %d",
   player->encoderData.video_codec_id,
   player->encoderData.video_frame_rate,
   player->encoderData.video_bit_rate,
   player->encoderData.video_width,
   player->encoderData.video_height,
   player->encoderData.duration);
   return 0;
...
int player_set_data_source(struct State *state, const char *file_path,
	...
	if ((err = player_fill_input_file_options(player)) < 0)
		goto error;
...
void jni_player_save(JNIEnv *env, jobject thiz, jstring string) {
    ...
    if (!player->playing) {
		LOGI(1, "jni_player_save could not save while not playing");
		throw_exception(env, not_playing_exception_class_path_name,
				"Could not save while not playing");
        pthread_mutex_unlock(&player->mutex_operation);
		goto end;
	}
    ...
    player->encoded_frame = 0;
    pthread_mutex_unlock(&player->mutex_operation);
    jni_player_pause(env, thiz);
    jni_player_seek(env, thiz, 0);


    pthread_mutex_lock(&player->mutex_save);
    // body
    //editor_main(file_path);

    editor_init(&player->encoderData, file_path);
    player->save = TRUE;
    jni_player_resume(env, thiz);
    // wait file finish
    while(player->save) {
        pthread_cond_wait(&player->cond_finish, &player->mutex_save);
    }
    editor_finish(&player->encoderData);
...
int jni_player_init(JNIEnv *env, jobject thiz) {
	...
	pthread_mutex_init(&player->mutex_save, NULL);

	pthread_cond_init(&player->cond_queue, NULL);
	pthread_cond_init(&player->cond_finish, NULL);
...
void jni_player_test_log(JNIEnv *env, jobject thiz) {
   LOGI(2, "!!! Test !!!");
}

void jni_player_ffmpeg_cmd(JNIEnv *env, jobject thiz, jobjectArray stringArray){
     int stringCount = (*env)->GetArrayLength(env, stringArray);
     const char *argv[stringCount];
     jstring jargv[stringCount];
    int i;
    for (i=0; i<stringCount; i++) {
        jargv[i] = (jstring) (*env)->GetObjectArrayElement(env, stringArray, i);
        argv[i] = (*env)->GetStringUTFChars(env, jargv[i], 0);
        // Don't forget to call `ReleaseStringUTFChars` when you're done.
        LOGI(1, "cccode: %s", argv[i]);

    }
    //main(stringCount, (char **)argv);
    for (i=0; i<stringCount; i++) {
